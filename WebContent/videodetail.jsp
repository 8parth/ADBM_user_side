<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	pageEncoding="ISO-8859-1"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=ISO-8859-1">
<title>Home: Welcome to Vidstream</title>

<link rel="stylesheet" type="text/css" href="css/style_2.css" />
<script type="text/javascript" src="js/jquery.min_2.js"></script>
<script type="text/javascript" src="js/cufon-yui_2.js"></script>
<script type="text/javascript" src="js/arial_2.js"></script>
<script type="text/javascript" src="js/switch_2.js"></script>

</head>
<body class="bg-body-v">
	<!--Header Section -->
	<%@include file="user-header.jsp" %>
	<!--Header Section  End-->

	<!--Content Section -->

	<div id="content_sec">
		<div id="left_content">
			<div id="banner_slider">
				<div id="title-video">
					<h1>Waka Waka (This Time for Africa)</h1>
					<div class="uploader-info">
						<img src="images/img-usr-thumb_2.gif" alt="" /><a
							href="videodetail.jsp#">by: shakiraVEVO</a>
					</div>
				</div>
				<div class="video-slider">
					<object type="application/x-shockwave-flash"
						style="width: 629px; height: 353px;"
						data="http://www.youtube.com/v/NKu0t0mVtNk?fs=1&amp;hl=en_US&amp;rel=0">
						<param name="movie"
							value="http://www.youtube.com/v/NKu0t0mVtNk?fs=1&amp;hl=en_US&amp;rel=0" />
						<param value="application/x-shockwave-flash" name="type" />
						<param value="true" name="allowfullscreen" />
						<param value="always" name="allowscriptaccess" />
						<param value="opaque" name="wmode" />
					</object>
				</div>
			</div>
			<div class="panel-video">
				<div class="panel-left">
					<ul>
						<li><a href="videodetail.jsp#" class="panl-1">Like</a></li>
						<li><a href="videodetail.jsp#" class="panl-3">Add to Playlist</a></li>
						<li><a href="videodetail.jsp#" class="panl-4">comments</a></li>
						
					</ul>
				</div>
				<div class="panel-right">
				<ul>
					<strong><li><a href="videodetail.jsp#" class="panl-5">Report as abuse</a></li></strong>
				</ul></div>
			</div>
			
			<div class="video-info">
				<div class="left-info-box">
					<ul>
						<li class="nobg">Uploaded by <a href="videodetail.jsp#">shakiraVEVO</a>
							on Jun 4, 2010, 08:20 PM
						</li>
						<li>Category: <a href="videodetail.jsp#">Music</a></li>
					</ul>
					<p>
						Video Details: <strong>1280x720, 127.61MB</strong>
					</p>
				</div>
				<div class="right-info-box">
					<p>
						<strong>294,752</strong> likes, <strong>15,084</strong> dislikes
					</p>
					
				</div>
			</div>
			<div id="comment-area">
				<div class="heading-comment">
					<h2>
						All Comments <strong>(4,033,360)</strong>
					</h2>
				</div>
				<div class="comment-list">
					<ul>
						<li><span class="img-area"> <img
								src="images/img-usr-small-1_2.gif" alt="" />
						</span>
							<div class="text-area">
								<h4>
									<a href="videodetail.jsp#">Keran Smith</a> <span>Says:</span>
								</h4>
								<p class="date-time">March 06, 2011 at 1:02 am</p>
								<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit.
									Vivamus dictum congue nisl. Duis ante nibh, porta quis imperdi
									et id, pulvinar at turpis. Donec vitae nunc sed tortor egestas
									blandit. Integer lorem risus, pellentesque eu interdum non,
									commodo nec sem. Quisque justo mauris,</p>
								<div class="like-panel">
									<a href="videodetail.jsp#" class="like-1">like</a> <a
										href="videodetail.jsp#" class="like-2">dislike</a> <a
										href="videodetail.jsp#" class="like-3">Replay</a>
								</div>
							</div></li>
						<li class="nobg"><span class="img-area"> <img
								src="images/img-usr-small-1_2.gif" alt="" />
						</span>
							<div class="text-area">
								<h4>
									<a href="videodetail.jsp#">Keran Smith</a> <span>Says:</span>
								</h4>
								<p class="date-time">March 06, 2011 at 1:02 am</p>
								<p>
									Lorem ipsum dolor sit amet, consectetur adipiscing elit.
									Vivamus dictum congue nisl. Duis ante nibh, porta quis
									imperdiet id, pulvinar at turpis. Donec vitae nunc sed tortor
									egestas blandit. Integer lorem risus, pellentesque eu interdum
									non, commodo nec sem. Quisque justo mauris, viverra ut mattis
									in <a href="videodetail.jsp#">http://www.youtube.com/watch?v=kffacxfA7G4</a>
									Rnare quis dui. Praesent pellentesque suscipit adipiscing.
									Vivamus iaculis libero ac tellus commodo convallis. Nam vel
									lacus ipsum, sed auctor tellus.
								</p>
								<div class="like-panel">
									<a href="videodetail.jsp#" class="like-1">like</a> <a
										href="videodetail.jsp#" class="like-2">dislike</a> <a
										href="videodetail.jsp#" class="like-3">Replay</a>
								</div>
							</div></li>
						<li><span class="img-area"> <img
								src="images/img-usr-small-2_2.gif" alt="" /></span>
							<div class="text-area">
								<h4>
									<a href="videodetail.jsp#">Keran Smith</a> <span>Says:</span>
								</h4>
								<p class="date-time">March 06, 2011 at 1:02 am</p>
								<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit.
									Vivamus dictum congue nisl. Duis ante nibh, porta quis imperdi
									et id, pulvinar at turpis. Donec vitae nunc sed tortor egestas
									blandit. Integer lorem risus, pellentesque eu interdum non,
									commodo nec sem. Quisque justo mauris,</p>
								<div class="like-panel">
									<a href="videodetail.jsp#" class="like-1">like</a> <a
										href="videodetail.jsp#" class="like-2">dislike</a> <a
										href="videodetail.jsp#" class="like-3">Replay</a>
								</div>
							</div></li>
						<li class="nobg"><span class="img-area"> <img
								src="images/img-usr-small-3_2.gif" alt="" />
						</span>
							<div class="text-area">
								<h4>
									<a href="videodetail.jsp#">Keran Smith</a> <span>Says:</span>
								</h4>
								<p class="date-time">March 06, 2011 at 1:02 am</p>
								<p>
									Lorem ipsum dolor sit amet, consectetur adipiscing elit.
									Vivamus dictum congue nisl. Duis ante nibh, porta quis
									imperdiet id, pulvinar at turpis. Donec vitae nunc sed tortor
									egestas blandit. Integer lorem risus, pellentesque eu interdum
									non, commodo nec sem. Quisque justo mauris, viverra ut mattis
									in <a href="videodetail.jsp#">http://www.youtube.com/watch?v=kffacxfA7G4</a>
									Rnare quis dui. Praesent pellentesque suscipit adipiscing.
									Vivamus iaculis libero ac tellus commodo convallis. Nam vel
									lacus ipsum, sed auctor tellus.
								</p>
								<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit.
									Vivamus dictum congue nisl. Duis ante nibh, porta quis
									imperdiet id, pulvinar at turpis. Donec vitae nunc sed tortor
									egestas blandit. Integer lorem risus, pellentesque eu interdum
									non, commodo nec sem. Quisque justo mauris.</p>
								<div class="like-panel">
									<a href="videodetail.jsp#" class="like-1">like</a> <a
										href="videodetail.jsp#" class="like-2">dislike</a> <a
										href="videodetail.jsp#" class="like-3">Replay</a>
								</div>
							</div></li>
						<li><span class="img-area"> <img
								src="images/img-usr-small-1_2.gif" alt="" />
						</span>
							<div class="text-area">
								<h4>
									<a href="videodetail.jsp#">Keran Smith</a> <span>Says:</span>
								</h4>
								<p class="date-time">March 06, 2011 at 1:02 am</p>
								<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit.
									Vivamus dictum congue nisl. Duis ante nibh, porta quis imperdi
									et id, pulvinar at turpis. Donec vitae nunc sed tortor egestas
									blandit. Integer lorem risus, pellentesque eu interdum non,
									commodo nec sem. Quisque justo mauris,</p>
								<div class="like-panel">
									<a href="videodetail.jsp#" class="like-1">like</a> <a
										href="videodetail.jsp#" class="like-2">dislike</a> <a
										href="videodetail.jsp#" class="like-3">Replay</a>
								</div>
							</div></li>
					</ul>
				</div>
			</div>
		</div>
		<div id="right_content">
			<div class="sign-in-field">
				<input type="text" value="Search Video" class="txt-field"
					onfocus="if(this.value == 'Search Video') { this.value = ''; }"
					onblur="if(this.value == '') { this.value = 'Search Video'; }"
					name="s" /> <a href="videodetail.jsp#">Search Button</a>
			</div>
			<div id="accordion">
				<h3 class="accordionButton">
					<a href="javascript: void(0);">Suggestions</a>
				</h3>
				<div class="trending-box accordionContent">
					<ul>
						<li>
							<div class="video-box-trending">
								<img src="images/img-tr-1_2.jpg" alt="" /> <span
									class="time-video">08:53</span> <span class="add-playlist"></span>
							</div>
							<div class="text-desc-tre">
								<h5>
									<a href="videodetail.jsp">Duis molestie nunc nec</a>
								</h5>
								<p>
									<strong>1,197,002</strong> views
								</p>
								<span>By <a href="videodetail.jsp#">Simon Cole</a></span>
							</div>
						</li>
						<li>
							<div class="video-box-trending">
								<img src="images/img-tr-2_2.jpg" alt="" /> <span
									class="time-video">08:53</span> <span class="add-playlist"></span>
							</div>
							<div class="text-desc-tre">
								<h5>
									<a href="videodetail.jsp">Duis molestie nunc nec</a>
								</h5>
								<p>
									<strong>1,197,002</strong> views
								</p>
								<span>By <a href="videodetail.jsp#">Simon Cole</a></span>
							</div>
						</li>
						<li>
							<div class="video-box-trending">
								<img src="images/img-tr-3_2.jpg" alt="" /> <span
									class="time-video">08:53</span> <span class="add-playlist"></span>
							</div>
							<div class="text-desc-tre">
								<h5>
									<a href="videodetail.jsp">Duis molestie nunc nec</a>
								</h5>
								<p>
									<strong>1,197,002</strong> views
								</p>
								<span>By <a href="videodetail.jsp#">Simon Cole</a></span>
							</div>
						</li>
						<li>
							<div class="video-box-trending">
								<img src="images/img-tr-4_2.jpg" alt="" /> <span
									class="time-video">08:53</span> <span class="add-playlist"></span>
							</div>
							<div class="text-desc-tre">
								<h5>
									<a href="videodetail.jsp">Duis molestie nunc nec</a>
								</h5>
								<p>
									<strong>1,197,002</strong> views
								</p>
								<span>By <a href="videodetail.jsp#">Simon Cole</a></span>
							</div>
						</li>
					</ul>
				</div>
				<h3 class="accordionButton">
					<a href="javascript: void(0);">Recent Views</a>
				</h3>
				<div class="trending-box accordionContent">
					<ul>
						<li>
							<div class="video-box-trending">
								<img src="images/img-tr-1_2.jpg" alt="" /> <span
									class="time-video">08:53</span> <span class="add-playlist"></span>
							</div>
							<div class="text-desc-tre">
								<h5>
									<a href="videodetail.jsp">Duis molestie nunc nec</a>
								</h5>
								<p>
									<strong>1,197,002</strong> views
								</p>
								<span>By <a href="videodetail.jsp#">Simon Cole</a></span>
							</div>
						</li>
						<li>
							<div class="video-box-trending">
								<img src="images/img-tr-2_2.jpg" alt="" /> <span
									class="time-video">08:53</span> <span class="add-playlist"></span>
							</div>
							<div class="text-desc-tre">
								<h5>
									<a href="videodetail.jsp">Duis molestie nunc nec</a>
								</h5>
								<p>
									<strong>1,197,002</strong> views
								</p>
								<span>By <a href="videodetail.jsp#">Simon Cole</a></span>
							</div>
						</li>
						<li>
							<div class="video-box-trending">
								<img src="images/img-tr-3_2.jpg" alt="" /> <span
									class="time-video">08:53</span> <span class="add-playlist"></span>
							</div>
							<div class="text-desc-tre">
								<h5>
									<a href="videodetail.jsp">Duis molestie nunc nec</a>
								</h5>
								<p>
									<strong>1,197,002</strong> views
								</p>
								<span>By <a href="videodetail.jsp#">Simon Cole</a></span>
							</div>
						</li>
						<li>
							<div class="video-box-trending">
								<img src="images/img-tr-4_2.jpg" alt="" /> <span
									class="time-video">08:53</span> <span class="add-playlist"></span>
							</div>
							<div class="text-desc-tre">
								<h5>
									<a href="videodetail.jsp">Duis molestie nunc nec</a>
								</h5>
								<p>
									<strong>1,197,002</strong> views
								</p>
								<span>By <a href="videodetail.jsp#">Simon Cole</a></span>
							</div>
						</li>
					</ul>
				</div>
				<h3 class="accordionButton">
					<a href="javascript: void(0);">Recent Tagged</a>
				</h3>
				<div class="trending-box accordionContent">
					<ul>
						<li>
							<div class="video-box-trending">
								<img src="images/img-tr-1_2.jpg" alt="" /> <span
									class="time-video">08:53</span> <span class="add-playlist"></span>
							</div>
							<div class="text-desc-tre">
								<h5>
									<a href="videodetail.jsp">Duis molestie nunc nec</a>
								</h5>
								<p>
									<strong>1,197,002</strong> views
								</p>
								<span>By <a href="videodetail.jsp#">Simon Cole</a></span>
							</div>
						</li>
						<li>
							<div class="video-box-trending">
								<img src="images/img-tr-2_2.jpg" alt="" /> <span
									class="time-video">08:53</span> <span class="add-playlist"></span>
							</div>
							<div class="text-desc-tre">
								<h5>
									<a href="videodetail.jsp">Duis molestie nunc nec</a>
								</h5>
								<p>
									<strong>1,197,002</strong> views
								</p>
								<span>By <a href="videodetail.jsp#">Simon Cole</a></span>
							</div>
						</li>
						<li>
							<div class="video-box-trending">
								<img src="images/img-tr-3_2.jpg" alt="" /> <span
									class="time-video">08:53</span> <span class="add-playlist"></span>
							</div>
							<div class="text-desc-tre">
								<h5>
									<a href="videodetail.jsp">Duis molestie nunc nec</a>
								</h5>
								<p>
									<strong>1,197,002</strong> views
								</p>
								<span>By <a href="videodetail.jsp#">Simon Cole</a></span>
							</div>
						</li>
						<li>
							<div class="video-box-trending">
								<img src="images/img-tr-4_2.jpg" alt="" /> <span
									class="time-video">08:53</span> <span class="add-playlist"></span>
							</div>
							<div class="text-desc-tre">
								<h5>
									<a href="videodetail.jsp">Duis molestie nunc nec</a>
								</h5>
								<p>
									<strong>1,197,002</strong> views
								</p>
								<span>By <a href="videodetail.jsp#">Simon Cole</a></span>
							</div>
						</li>
					</ul>
				</div>
			</div>
			<div class="classic-channel">
				<div class="heading-top">
					<h2>Classic Channels</h2>
				</div>
				<ul>
					<li><a href="videodetail.jsp#" class="red"> <img
							src="images/img-c1_2.jpg" alt="" /> <span>The woman in
								black</span> <em><strong>1,197,002</strong> Members</em></a></li>
					<li><a href="videodetail.jsp#" class="green"> <img
							src="images/img-c2_2.jpg" alt="" /> <span>The Intention</span> <em><strong>1,197,002</strong>
								Members</em>
					</a></li>
					<li><a href="videodetail.jsp#" class="silver"> <img
							src="images/img-c3_2.jpg" alt="" /> <span>Lenny</span> <em><strong>1,197,002</strong>
								Members</em></a></li>
				</ul>
			</div>
			<br class="clear" />
			<div class="trending-box">
				<div class="heading-top">
					<h2>Trending Now</h2>
					<a href="videodetail.jsp#">View More</a>
				</div>
				<ul>
					<li>
						<div class="video-box-trending">
							<img src="images/img-tr-1_2.jpg" alt="" /> <span
								class="time-video">08:53</span> <span class="add-playlist"></span>
						</div>
						<div class="text-desc-tre">
							<h5>
								<a href="videodetail.jsp">Duis molestie nunc nec</a>
							</h5>
							<p>
								<strong>1,197,002</strong> views
							</p>
							<span>By <a href="videodetail.jsp#">Simon Cole</a></span>
						</div>
					</li>
					<li>
						<div class="video-box-trending">
							<img src="images/img-tr-2_2.jpg" alt="" /> <span
								class="time-video">08:53</span> <span class="add-playlist"></span>
						</div>
						<div class="text-desc-tre">
							<h5>
								<a href="videodetail.jsp">Duis molestie nunc nec</a>
							</h5>
							<p>
								<strong>1,197,002</strong> views
							</p>
							<span>By <a href="videodetail.jsp#">Simon Cole</a></span>
						</div>
					</li>
					<li>
						<div class="video-box-trending">
							<img src="images/img-tr-3_2.jpg" alt="" /> <span
								class="time-video">08:53</span> <span class="add-playlist"></span>
						</div>
						<div class="text-desc-tre">
							<h5>
								<a href="videodetail.jsp">Duis molestie nunc nec</a>
							</h5>
							<p>
								<strong>1,197,002</strong> views
							</p>
							<span>By <a href="videodetail.jsp#">Simon Cole</a></span>
						</div>
					</li>
					<li>
						<div class="video-box-trending">
							<img src="images/img-tr-4_2.jpg" alt="" /> <span
								class="time-video">08:53</span> <span class="add-playlist"></span>
						</div>
						<div class="text-desc-tre">
							<h5>
								<a href="videodetail.jsp">Duis molestie nunc nec</a>
							</h5>
							<p>
								<strong>1,197,002</strong> views
							</p>
							<span>By <a href="videodetail.jsp#">Simon Cole</a></span>
						</div>
					</li>
				</ul>
			</div>
			<div class="event-box">
				<div class="heading-top">
					<h2>Events</h2>
					<a href="videodetail.jsp#">View More</a>
				</div>
				<ul>
					<li>
						<div class="img-frame-event">
							<img src="images/img-c1_2.jpg" alt="" />
						</div>
						<div class="text-desc-event">
							<h5>Duis molestie nunc les.</h5>
							<p>Duis molestie nunc nec felis sollicit udin sed ornare
								lorem varius.</p>
						</div>
					</li>
					<li>
						<div class="img-frame-event">
							<img src="images/img-c2_2.jpg" alt="" />
						</div>
						<div class="text-desc-event">
							<h5>Duis molestie nunc les.</h5>
							<p>Duis molestie nunc nec felis sollicit udin sed ornare
								lorem varius.</p>
						</div>
					</li>
					<li>
						<div class="img-frame-event">
							<img src="images/img-c3_2.jpg" alt="" />
						</div>
						<div class="text-desc-event">
							<h5>Duis molestie nunc les.</h5>
							<p>Duis molestie nunc nec felis sollicit udin sed ornare
								lorem varius.</p>
						</div>
					</li>
				</ul>
			</div>
		</div>
		<br class="clear" />
	</div>
	<!--Content Section End -->

	<!--Footer -->
	<%@include file="footer.jsp" %>
	<!--Bottom Footer Ends -->
</body>
</html>